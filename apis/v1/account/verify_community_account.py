...
class CommunityAccountListForm(Form):

    account = StringField(validators=[length(min=0, max=100, message='超出范围')])
    contributor = StringField(validators=[length(min=0, max=100, message='超出范围')])
    tip = StringField(validators=[length(min=0, max=100, message='超出范围')])
    status = StringField(validators=[AnyOf(['1','0','','premit'])],default='')
